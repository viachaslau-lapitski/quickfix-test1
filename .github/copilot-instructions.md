...
**Latency reporting**: Client reports `p95_ms` (95th percentile `sendToTarget` latency), `p99_ms` (99th percentile `sendToTarget` latency), and `p100_ms` (max `sendToTarget` latency) once per second using a Micrometer `Timer`.
